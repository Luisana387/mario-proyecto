...
const SonidoFinalizado = new Audio ("./sonidos/Game_Over.wav");
const SonidoPoder= new Audio ("./sonidos/Powerup.wav");
const SonidoVictoria = new Audio ("/sonidos/Level_Clear.wav");
const SonidoMuerto = new Audio ("./sonidos/Die.wav");
const SonidoPausa = new Audio ("/sonidos/Pause.wav");
...
window.addEventListener("keydown", (event) => {
  setTimeout(() => {
    if (event.keyCode == 37 && !izquierdaPresionado) {
      ...
      if(abajoPresionado == true){
        if( marioGrande== false){
...
function derecha() {
  if (derechaPresionado) {
    ...
    let marioActual2 =parseInt(mario.style.left) || 75;
    const mover= 10;
    const limite =1800;
     
    let marioNuevo2 =marioActual2 + mover;

   if (marioNuevo2 <=limite) {   
     mario.style.left = marioNuevo2+ "px" 
...
function agarrarMoneda() {
  if (moneda.style.display != "none") {
    ...
    let monedaPos = moneda.getBoundingClientRect(); // Posición de la bandera
  if(
...
function agarrarHongo() {
  if (hongo.style.display != "none") {
    ...
    let marioPos = mario.getBoundingClientRect(); // Posición de Mario en pantalla
    let hongoPos = hongo.getBoundingClientRect(); // Posición de la bandera
  if(
...
function detectarColisionBandera() {
  ...
  if (
    ...
    if (marioGrande && TieneMoneda) {
      ...
      SonidoVictoria.play(); // Reproduce sonido de victoria
...
botonFinalizarJuego.onclick = function () {
  setTimeout(() => {
   nivel.innerHTML="Juego Finalizado: Luisana";
  SonidoFinalizado.play(); // Agrega aquí la lógica para terminar el juego, como mostrar mensaje o reiniciar
  TemaFondo.pause();
  juegoActivo = false;
  }, 0);
};
...
function detectarColisionGoomba() {
  ...
  if (
    ...
    SonidoMuerto.play(); // Reproduce sonido de muerte
